...
X_DIM = 3
U_DIM = 1
PARAM_DIM = 8  # ndm: +2 for runtime comfort_brake / stop_distance
COST_E_DIM = 5
...
def gen_long_model():
  ...
  # ndm: runtime-tunable braking comfort limits (were compiled-in constants)
...
def gen_long_ocp():
  ...
  comfort_brake = ocp.model.p[6]  # ndm
  stop_distance = ocp.model.p[7]  # ndm

  ocp.cost.yref = np.zeros((COST_DIM, ))
  ocp.cost.yref_e = np.zeros((COST_E_DIM, ))

  desired_dist_comfort = get_safe_obstacle_distance(v_ego, lead_t_follow, comfort_brake, stop_distance)  # ndm
